...
df_clean %<>% mutate(across(generation.biomass:price.actual, .fns = na_interpolation, option = 'linear')) %>% 
  select(c(1, starts_with("generation")))  
long_gen_data <- df_clean %>% 
  pivot_longer(cols = -time, names_to = "generation_source") %>% 
  mutate(generation_source = str_replace(generation_source, "generation.", ""))

# Verify
long_gen_data[!complete.cases(long_gen_data),]
```
...
```{r}
# Proportions
df_props <- cbind(df_clean[1], prop.table(as.matrix(df_clean[-1]), margin = 1))
...
df_corr <- df_clean[1:10000,] %>%
  select(starts_with("generation")) 

names(df_corr) <- c("Biomass",
                    "Lignite",
                    "Gas",
                    "Hard Coal",
                    "Oil",
...
long_gen_data_props$year <- year(long_gen_data$time)
long_gen_data_props$month <- month(long_gen_data$time)
long_gen_data_props$day <- day(long_gen_data$time)
long_gen_data_props$hour <- hour(long_gen_data$time)
```
