...
```
```{r, include=TRUE, fig.width = 14, fig.height = 30, warning=FALSE, message = FALSE}
library(seastests)
library(ggfortify)
library(zoo)
library(tseries)
...
temp[[]] %>% 
  ggplot()+
  geom_line(aes(x = 1:length(temp[[2]]), y = normalize(as.numeric(temp[[2]]), method = "standardize", range = c(0, 1)), color = "Gas"))+
  geom_line(aes(x = 1:length(temp[[4]]), y = normalize(as.numeric(temp[[4]]), method = "standardize", range = c(0, 1)), color = "Hard Coal"))+
  geom_line(aes(x = 1:length(temp[[9]]), y = normalize(as.numeric(temp[[9]]), method = "standardize", range = c(0, 1)), color = "Lignite"))+
  geom_line(aes(x = 1:length(temp[[11]]), y = normalize(as.numeric(temp[[11]]), method = "standardize", range = c(0, 1)), color = "Oil")) + xlab("Day from start") + ylab("Normalised Power Generated")
